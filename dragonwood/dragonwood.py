...
def choose():  # this is like the main function where alot of the bulk of the selection happens
    while True:
        try:
            choose = input(
            ...
            if validate_input(choose) == True:
                ...
                if ad.process_scream():
                    print(
                        f"You {ad.attacktype()} at the monster with {ad.dice_rolled()} dice rolled!"
                    )
                    ...
                    if compare_attack(result, attack_num) == True:
                        ...
                        md.draw_to_landscape()
                        print(
                            f"Success you captured {mon.name} with an attack roll of {result} you only needed {attack_num}"
                        )
                        ...
                        print(
                            f"Failed to capture: {mon.name} required {attack_num} you only achieved {result}"
                        )
                        md.disp_landscape()
                ...
                ad.pick()
                print(
                    "Valid inputs are 'p' or 123, 5555, 6793 etc.... im going to give u are card."
                )
                ad.hand()
